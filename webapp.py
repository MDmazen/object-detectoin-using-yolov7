...
def predict_img():
    if request.method == "POST":
        if 'file' in request.files:
            ...
            if file_extension == 'jpg':
                process = Popen(["python", "detect.py", '--source', filepath, "--weights","epoch_078.pt"], shell=True)
                process.wait()
    ...
    return redirect('http://192.168.63.221/Object-Detection-Web-App-Using-YOLOv7-and-Flask-main/templates/result.php')
    #return "done"
...
if __name__ == "__main__":
    ...
    model = torch.hub.load('.', 'custom','epoch_078.pt', source='local')
    model.eval()
